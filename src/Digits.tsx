...
const Digits: React.FC = () => {
  ...
  const helpMe = () => {
    ...
    for (let move of currentSolution) {
      ...
      var currNumber = parseInt(operations[0]);
      
      for (var i = 0; i < operations.length; i+=2) {
        const signType = operations[i+1].substring(0, 1) === "*" ? "×" : operations[i+1].substring(0, 1);
        const num2 = parseInt(operations[i+1].slice(1));
        ...
        console.log("number1: " + prettyPrintNumber(number1) + " number 2 : " + prettyPrintNumber(number2));
        console.log("Sign obj " + prettyPrintSign(signObj));
        
        if (number1 && number2 && signObj) {     
          selectNumber(number1.id); 
          selectSign(signObj.id);
          selectNumber(number2.id);
          return;
  ...
  const selectNumber = (id: number) => {
    ...
    console.log("Selected Number: " + prettyPrintNumber(selectedNumber));
    console.log("Selcted Sign: " + prettyPrintSign(selectedSign));
    console.log("Current Number: " + prettyPrintNumber(currentNumber));
    ...
    if (selectedNumber && selectedSign && currentNumber) {
      ...
      console.log("Slecting number")
      updatedNumbers[currentPuzzleIndex] = numbers.map(number =>
        number.id === id ? { ...number, selected: !number.selected } : { ...number, selected: false } 
      );
      console.log(updatedNumbers[currentPuzzleIndex].map(prettyPrintNumber).join("\n"));
      setNumbersList(updatedNumbers);
    }
  };
  ...
  const selectSign = (id: string) => {
    ...
    console.log(signs.map(prettyPrintSign).join("\n"));
  };
