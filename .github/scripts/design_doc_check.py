...
docs/ — a PR that's already documentation doesn't need an ADR generated
about it. Re-run on every push to the PR (opened/synchronize/reopened):
regenerates from the current diff and force-pushes the same deterministic
branch, so there's only ever one ADR PR per source PR.
...
# ── Phase 0: skip PRs that are already documentation ────────────────────────
# A PR whose diff touches nothing outside docs/ doesn't need an ADR generated
# about it — it already is a doc. Checking this from the diff's file paths
# (rather than asking the model to judge) avoids the model reading a design
# doc's *content* and deciding the described change is ADR-worthy, which is
# what it will do if asked to judge significance from prose about a schema
# change instead of from an actual code diff.
...
if changed_paths and all(p.startswith("docs/") for p in changed_paths):
    print(
        ...
        "this PR is already documentation. Skipping ADR generation."
    )
    sys.exit(0)
...
# -B (not -b) + push --force: branch_name is deterministic (design-doc/pr-N), so a
# second run on the same PR (synchronize) must overwrite the prior attempt rather
# than fail trying to re-create a branch that already exists on the remote from the
# first run. Same pattern wiki_update_pr.py already uses for its wiki-update branch.
...
if existing_pr:
    print(f"ADR PR already open: {repo_full}#{existing_pr} — updated in place.")
else:
...
"""
    ...
    print(f"PR opened: {repo_full} — {branch_name}")
